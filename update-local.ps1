...
if (-not (Test-Path "scraper\node_modules")) {
  Write-Host "[update-local] installing deps..."
...
function Resolve-Rebase {
  ...
  if ($code -ne 0) {
    ...
    Write-Host "[update-local] rebase failed, skip"
    exit 0
  }
}

Write-Host "[update-local] pulling..."
$code = Git @("pull", "--rebase", "origin", "main")
if ($code -ne 0) {
  Resolve-Rebase
}

Write-Host "[update-local] scraping..."
node "scraper\scrape.mjs" --mode=browser
if ($LASTEXITCODE -ne 0) {
  Write-Host "[update-local] scrape failed, exit=$LASTEXITCODE"
  exit 1
}

Git @("add", "data/weather.json", "data/iot") | Out-Null
if (Git @("diff", "--cached", "--quiet") -eq 0) {
  Write-Host "[update-local] no data change"
} else {
  Git @("commit", "-m", "chore: update weather data (local)")
}

Write-Host "[update-local] pulling again..."
$code = Git @("pull", "--rebase", "origin", "main")
if ($code -ne 0) {
  Resolve-Rebase
}
