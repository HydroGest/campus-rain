...
async function fetchOpenMeteoNowcast(lat, lng) {
  ...
  return {
    ...
    probability: probs.slice(0, 8).map((p) => Number(p) / 100),
    rainyMinutes2h: rainy.slice(0, 8).filter(Boolean).length * 15,
    dryMinutes2h: rainy.slice(0, 8).filter((v) => !v).length * 15,
